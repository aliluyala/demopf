...
Ext.onReady(function(){
    ...
    var store = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            ...
            method:'POST',
        	type: 'ajax',
            url: '../../sys/proxy/insuranceCheckDDByPage.action',
            reader: {
                root: 'root',
			...
			listeners : {
			       afterRender : function(combo) {
			    	   if(this.getValue()==null){
						   ...
						   Ext.getCmp('photoId').setVisible(true);
			    	   }
			       },
					select : function(combo,record){
						if(combo.getValue()==true){
							...
							Ext.getCmp('photoId').setValue(" ");
							Ext.getCmp('photoId').setVisible(false);
						}
					}
			}
		},
	    ...
	    {  
	      ...
	      buttons:[{  
	        	  ...
	        	  handler: function() {
	        		  ...
	        		  var check = Ext.getCmp('checked').getValue();
	        		  var photo = Ext.getCmp('photoId').getValue();
	        		  if(check&&(photo==""||photo==null)){
	        			  Ext.Msg.alert('操作失败', "上传保单");
	        			  return;
	        		  }
	                  if (form.isValid()) {
	                      form.submit({
