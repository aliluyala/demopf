...
Ext.onReady(function(){
    ...
    Ext.define('travelModel', {
        ...
        fields: ['orderNo','policyNo','recognizeePhone','goodsName','weigthOrCount','packQty','cargoNo','goodsTypeNo','mainGlausesCode','insuredAmount','truckType',
                 ...
                 'company','status','created_date','policyPath','tPolicyNo'
        ],
        idProperty: 'id'
    });
    ...
    var store = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            ...
            method:'POST',
        	type: 'ajax',
            url: '../../sys/proxy/insuranceDDByPage.action',
            reader: {
                root: 'root',
	    ...
	    items: ['-',
	            {
			        ...
			        handler: function () {
			        	...
			        	if(records[0].data.status!='完成交易'){
			        		Ext.MessageBox.alert("提示","不是完成交易状态,不能撤单！");
			        		return;
			        	}
			        	Ext.MessageBox.confirm("提示","确定要撤单吗？",function(but){
			        		if(but=='yes'){
			        			Ext.Ajax.request({url:'../../sys/proxy/cancleDDInsurance.action',  
	            ...
	            '-',{
			        ...
			        handler: function () {
	                	...
	                	if(p!=null&&p!=""){
	                		window.location=p;
	                		/*Ext.getCmp("path").update('<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="'+p+'"></iframe>');
        	...
        	listeners: {
                itemdblclick: function (me, record, item, index, e, eOpts) {
                	Ext.getCmp('id2').setValue(record.data.id);
			  		...
			  		Ext.getCmp('tPolicyNo').setValue(record.data.tPolicyNo);
			  		var h = Ext.getBody().getHeight();
