...
public class MD5 {
	...
	private byte buffer[];
	private byte digest[];
	private static final char[] HEX_DIGIST = new char[] { '0', '1', '2', '3',
	...
	public synchronized String getSHA1(String s) {
		String s2 = "";
		try {
			byte abyte0[] = s.getBytes("utf8");
			MessageDigest messagedigest = MessageDigest.getInstance("sha-1");
			messagedigest.update(abyte0, 0, abyte0.length);
			s2 = toHexString(messagedigest.digest(), false);
		} catch (NoSuchAlgorithmException nsaex) {
			nsaex.printStackTrace();
		} catch (UnsupportedEncodingException neex) {
	...
	public static long b2iu(byte byte0) {
		return (long) (byte0 < 0 ? byte0 & 0xff : byte0);
	}
	...
	public static String toHexString(byte abyte0[], boolean flag) {
		...
		if (abyte0 != null) {
			...
			for (int i = 0; i < abyte0.length; i++) {
				byte byte0 = abyte0[i];
				ac[2 * i] = HEX_DIGIST[0xf & byte0 >>> 4];
				ac[2 * i + 1] = HEX_DIGIST[byte0 & 0xf];
			}
			s = new String(ac);
			if (flag)
				s = s.toUpperCase();
	...
	public static void main(String args[]) {
		...
		if (Array.getLength(args) == 0) {
			...
			System.out.println("MD5(\"\"):" + md5.getMD5(""));
			System.out.println("MD5(\"123456\"):" + md5.getMD5("123456"));
			System.out.println("MD5(\"a\"):" + md5.getMD5("a"));
			System.out.println("MD5(\"abc\"):" + md5.getMD5("abc"));
			System.out.println("MD5(\"message digest\"):"
					+ md5.getMD5("message digest"));
			System.out.println("MD5(\"abcdefghijklmnopqrstuvwxyz\"):"
					+ md5.getMD5("abcdefghijklmnopqrstuvwxyz"));
			System.out
					.println("MD5(\"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789\"):"
							+ md5.getMD5("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"));
		} else {
			System.out.println("MD5(" + args[0] + ")=" + md5.getMD5(args[0]));
		}
	}
}